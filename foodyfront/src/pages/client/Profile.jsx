...
import { User, Home, ShoppingBag, Calendar, LogOut, Edit, MapPin, Phone, Mail } from 'lucide-react';
...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      console.log("Voici ", user);
      if (token && user) {
        try {
          setIsAuthenticated(true);
          const response = await getUser(user.id || user.userId, 'CLIENT');
        ...
        <nav className="mt-6">
          <ul>
            <li className="px-4 py-3 hover:bg-yellow-50 cursor-pointer transition-all duration-300 border-l-4 border-transparent hover:border-yellow-500">
              <a href="/client" className="flex items-center text-gray-700">
                <Home size={20} className="mr-3" />
                <span>Accueil</span>
              </a>
            </li>
            <li className="px-4 py-3 hover:bg-yellow-50 cursor-pointer transition-all duration-300 border-l-4 border-transparent hover:border-yellow-500">
              <a href="/client/commandes" className="flex items-center text-gray-700">
                <ShoppingBag size={20} className="mr-3" />
                <span>Mes Commandes</span>
              </a>
            </li>
            <li className="px-4 py-3 hover:bg-yellow-50 cursor-pointer transition-all duration-300 border-l-4 border-transparent hover:border-yellow-500">
              <a href="/client/reservations" className="flex items-center text-gray-700">
